...
describe('<ExonForm />', () => {
  it('renders an exon form', () => {
    const wrapper = shallow(<ExonForm onCancel={() => {}} />);
  ...
  it('renders an new empty exon form', () => {
    const wrapper = shallow(<ExonForm onCancel={() => {}} />);
    expect(
      wrapper.find('input[type="number"]').get(0).props.value.length
    ).to.equal(0);
    expect(
      wrapper.find('input[type="number"]').get(1).props.value.length
    ).to.equal(0);
    expect(
  ...
  it('cancels exon form display', () => {
    const spy = sinon.spy();
    const wrapper = shallow(<ExonForm onCancel={spy} />);
  ...
  it('submits exon creation form', () => {
    ...
    const wrapper = shallow(
      <ExonForm onCreateNewExon={spy} onCancel={() => {}} />
    );
