export var Nt = (function() {
  ...
  void (function() {
    ...
    for (var i = 0; i < len; i++) {
      ...
      for (var j = 0; j < len; j++) {
        ...
        a[ki + kj] = byte;
        b[byte] =
          __nucleotideTo4Bit[complementNucleotide(kj)] |
          (__nucleotideTo4Bit[complementNucleotide(ki)] << 4);
        c[byte] = ki + kj;
        d[byte] = [ki, kj];
      }
    }
  ...
  Seq.prototype.complement = function() {
    ...
    var complement = new Seq(this.__type).readBuffer(
      this.__complement.slice(0)
    );
  ...
  Seq.prototype.polymerize = function(seq) {
    ...
    if (shift) {
      ...
      for (var i = 0, len = seqArray.length; i < len; i++) {
        newArray[++copyPos] =
          (seqArray[i] >>> shiftSeq) | (seqArray[i + 1] << shift);
      }
    } else {
      for (var i = 0, len = seqArray.length; i < len; i++) {
  ...
  Seq.prototype.__slice = function(start, length) {
    ...
    var subBuffer = this.__buffer.slice(
      4 + (start >>> 1),
      4 + (start >>> 1) + Math.ceil(length * 2)
    );
    ...
    if (start & 1) {
      ...
      if (remainder) {
        ...
        for (var i = 0, len = subRemainderArray.length; i < len; i++) {
          remainderArray[i] =
            (subRemainderArray[i] >>> 4) | (subRemainderArray[i + 1] << 4);
        }
      }
    } else {
      for (var i = 0, len = subArray.length; i < len; i++) {
  ...
  MatchResult.prototype.alignment = function() {
    if (!this.__align) {
      ...
      this.__align = map.__searchSpace.replicate(
        this.position,
        map.__query.__length
      );
    }
    return this.__align;
  };
  ...
  MatchMap.prototype.__initialize = function() {
    ...
    var dataArray = new Uint32Array(
      this.__execute(this.__query.__buffer, this.__searchSpace.__buffer)
    );
  ...
  MatchMap.prototype.bottom = function(n) {
    ...
    return this.__orderedResults
      .slice(this.__orderedresults.length - n, n)
      .map(function(v) {
        return new MatchResult(self, v.pos, v.matches);
      });
  };
